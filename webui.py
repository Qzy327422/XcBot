...
def save_config_bundle(data: Dict[str, Any]):
    ...
    if isinstance(raw_providers, list):
        ...
        for index, provider in enumerate(raw_providers, start=1):
            ...
            if not provider_id:
                raise ValueError(f"第 {index} 个渠道的 ID 不能为空")
            ...
            if isinstance(raw_models, list):
                for model_index, model_cfg in enumerate(raw_models, start=1):
                    if not isinstance(model_cfg, dict):
                        continue
                    model_name = str(model_cfg.get("name", "") or model_cfg.get("model", "") or "").strip()
                    if not model_name:
                        raise ValueError(f"渠道 {provider_id} 的第 {model_index} 个模型名称不能为空")
