...
def _get_resource_usage() -> Dict[str, Any]:
    ...
    try:
        ...
        # interval=None 是与上次调用之间的差值，首次调用会返回 0.0
        percent = round(proc.cpu_percent(interval=None), 1)
        return {"cpu_percent": percent, "memory_mb": _get_memory_usage_mb()}
    except Exception:
        pass
    ...
    with _resource_lock:
        ...
        _resource_sample.update({"wall": now, "cpu": cpu_now, "percent": percent})
    return {"cpu_percent": percent, "memory_mb": _get_memory_usage_mb()}
...
def _get_github_download_mirrors() -> list[str]:
    """GitHub 下载加速前缀。为空字符串表示直连。

    用户可在 config.json 里配置：
      Others.github_download_mirrors = ["", "https://gh.llkk.cc/", "https://ghproxy.net/"]

    每个镜像会按 `prefix + github_url` 拼接。默认保留直连 + 常见镜像；
    某个镜像失效时会自动尝试下一个。
    """
    defaults = [
        "",
