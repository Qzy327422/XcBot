...
def build_ui_schema(cfg: Dict[str, Any]) -> list[Dict[str, Any]]:
    return [
        ...
        {"key": "debug", "title": "调试", "icon": "🛠️", "desc": "运行资源、缓存和会话状态", "fields": []},
...
def _debug_runtime_snapshot() -> Dict[str, Any]:
    ...
    try:
        ...
        if nickname_cache is not None:
            runtime["nickname_cache"] = {"items": _safe_len(nickname_cache)}

        chat_db = getattr(main_mod, "chat_db", None)
        if isinstance(chat_db, dict):
            group_rows = []
            total_history = 0
            for group_id, data in list(chat_db.items())[:20]:
            ...
            runtime["chat_db"] = {"groups": _safe_len(chat_db), "history_total_sampled": total_history, "top_groups": group_rows}
        ...
        contexts = getattr(main_mod, "contexts", None)
        if contexts is not None:
            runtime["contexts"] = {"items": _safe_len(contexts)}

        context_manager = getattr(main_mod, "context_manager", None)
        if context_manager is not None:
            runtime["context_manager"] = {
                "private_contexts": _safe_len(getattr(context_manager, "privates", {})),
                "group_contexts": _safe_len(getattr(context_manager, "groups", {})),
            }
...
const publicRoute={welcome:'welcome',stats:'stats',bot:'config',ai:'ai',persona:'persona',features:'features',security:'security',connection:'connection',webui:'webui',debug:'debug',logs:'logs'};
...
function applyRoute(){const r=splitHashRoute();let page=normalizeRoutePage(r.page);if(pages().length&&!pages().some(p=>p.key===page))page='welcome';state.current=page;state.section=r.section||'';localStorage.webuiPage=page}
...
function gotoPage(k,section=''){if(state.bundle&&!pages().some(p=>p.key===k))k='welcome';syncCurrentPageFieldsFromDom();updateRoute(k,section);render()}
...
function renderDebug(){const d=state.bundle.status?.debug||{},ru=d.resource_usage||{},rt=d.runtime||{},gc=d.gc||{},web=d.webui||{};const procCpu=ru.cpu_percent_process??ru.cpu_percent??0,normCpu=ru.cpu_percent_normalized??ru.cpu_percent??0,sysCpu=ru.cpu_percent_system??'--';const rows=[['PID',state.bundle.status?.pid],['进程CPU(整机口径)',`${normCpu}%`],['进程CPU(单核口径)',`${procCpu}%`],['系统总CPU',`${sysCpu}%`],['CPU逻辑核心',ru.cpu_count??'--'],['RSS 内存',`${ru.rss_mb??ru.memory_mb??0} MB`],['虚拟内存',`${ru.vms_mb??0} MB`],['线程数',ru.threads??'--'],['打开文件',ru.open_files??'--'],['网络连接',ru.connections??'--'],['日志缓冲',web.log_buffer??0],['GC 对象',gc.tracked_objects??'--'],['GC 计数',(gc.counts||[]).join(' / ')]];const cacheRows=[['昵称缓存',rt.nickname_cache?.items],['chat_db 群数',rt.chat_db?.groups],['chat_db 抽样历史',rt.chat_db?.history_total_sampled],['Token 会话',rt.token_stats?.sessions],['Token 用户',rt.token_stats?.users],['Token 群',rt.token_stats?.groups],['上下文对象',rt.contexts?.items],['私聊上下文',rt.context_manager?.private_contexts],['群聊上下文',rt.context_manager?.group_contexts]];const groups=rt.chat_db?.top_groups||[];return `<div class="card"><div class="section-head"><div><h3>进程资源</h3><p>当前 Python 进程的实时采样</p></div><button class="btn" onclick="refreshAll(true)">刷新调试数据</button></div><div class="mini-stats"><div class="stat"><b>${esc((ru.rss_mb??ru.memory_mb??0)+' MB')}</b><span>RSS 内存</span></div><div class="stat"><b>${esc(normCpu+'%')}</b><span>进程CPU(整机)</span></div><div class="stat"><b>${esc(procCpu+'%')}</b><span>进程CPU(单核)</span></div><div class="stat"><b>${esc(sysCpu+'%')}</b><span>系统总CPU</span></div><div class="stat"><b>${esc(ru.threads??'--')}</b><span>线程</span></div><div class="stat"><b>${esc(web.log_buffer??0)}</b><span>日志缓冲</span></div></div></div><div class="card half"><div class="section-head"><div><h3>运行快照</h3><p>${esc(d.generated_at?new Date(d.generated_at*1000).toLocaleString():'--')}</p></div></div><div class="kv">${rows.map(r=>`<div>${esc(r[0])}</div><div>${esc(r[1]??'不可用')}</div>`).join('')}</div></div><div class="card half"><div class="section-head"><div><h3>缓存/会话</h3><p>用于定位跑久后变胖的对象</p></div></div><div class="kv">${cacheRows.map(r=>`<div>${esc(r[0])}</div><div>${esc(r[1]??'不可用')}</div>`).join('')}</div></div><div class="card"><div class="section-head"><div><h3>chat_db 抽样</h3><p>最多显示前 20 个群的历史条数</p></div></div>${groups.length?`<div style="display:grid;gap:10px">${groups.map(g=>`<div class="field" style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><b>${esc(g.group)}</b><span class="pill">${esc(g.history)} 条历史</span><span class="pill">${esc(g.tokens)} tokens</span></div>`).join('')}</div>`:'<div class="desc">暂无 chat_db 数据或当前模块不可见。</div>'}${d.runtime_error?`<pre class="log" style="margin-top:14px">${esc(d.runtime_error)}</pre>`:''}</div>`}
...
setTheme(localStorage.webuiTheme||'dark');window.addEventListener('hashchange',()=>{applyRoute();render()});applyRoute();if(!location.hash)updateRoute(state.current,'',true);refreshAll(true);setInterval(()=>refreshAll(false),3000);setInterval(()=>{if(state.current==='welcome'||state.current==='logs')scrollLogsToBottom()},500);
</script></body></html>'''
