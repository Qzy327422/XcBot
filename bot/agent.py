...
# 当前进程的运行环境，供 build_agent_guide 告知模型 shell 语法。
...
SHELL_HINT = (
    "当前 shell 是 Windows 命令提示符（cmd.exe），请使用 cmd 兼容的命令，"
    "不要假设 cat / ls / grep 这类 Unix 命令可用。"
    if _OS_NAME.lower() == "windows"
    else "当前 shell 是类 Unix 环境，请使用 POSIX 兼容的命令。"
)


def build_agent_guide(now_text: str, is_group: bool, user_level: str,
                      tool_names: list[str], workspace: str = "",
                      os_name: str = "", shell_hint: str = "") -> str:
    """生成注入系统提示词的工具使用指南。

    措辞照搬 AstrBot 已验证过的几条（astr_main_agent_resources.py:25 TOOL_CALL_PROMPT、
    astr_main_agent.py:439 _build_local_mode_prompt、:1631 workspace 约束），翻成中文并
    补上 QQ 场景特有的部分。

    原则和 AstrBot 一致：只提供模型判断所需的事实（当前时间、环境、可用工具、格式约定），
    具体何时调什么工具交给模型自己决定，不在代码里做自然语言解析之类的猜测。
    """
    lines = [
        "# 工具使用",
        f"当前时间：{now_text}。这是真实时间，涉及日期计算时以此为准，不要凭记忆猜。",
        f"当前场景：{'群聊' if is_group else '私聊'}。"
        f"发起者身份：{'管理员（可使用全部已开启的工具）' if user_level == 'admin' else '普通用户（部分工具无权调用）'}。",
        "",
        # 对应 AstrBot TOOL_CALL_PROMPT：这四条是它反复调过的核心约束
        "## 基本规则",
        "- 绝不返回空回复，调用工具之后尤其要给用户一句话。",
        "- 开始一类新任务时简短说明你要做什么，但不必在每次调用工具前都报备一遍。",
        "- 严格按工具的参数定义填写，不要编造不存在的参数或猜测参数格式。",
        "- 调用工具前后都保持人格和说话风格一致，不要切换成公文腔。",
        "- 工具返回以 error 开头的内容时，把失败原因如实告诉用户，不要假装成功。",
        "",
        "## 何时该用工具",
        "你可以自主决定是否调用、调用哪个、调用几次，不需要请求许可。",
        "- 需要实时信息或你不确定的事（新闻、天气、价格、版本号、赛果）时，先查证再回答，不要凭印象编造。",
        "- 一次可以同时请求多个工具，它们会并发执行，比一个个来更快。",
        "- 多步任务先想清楚顺序再动手，比如「查完 A 再根据结果查 B」，不要盲目重复同一次调用。",
    ]

    # future_task 的时间格式没法只靠参数描述讲清（要模型自己换算自然语言），
    # 所以留在提示词里；但只在它真的可用时才讲，否则是凭空多出来的概念。
    if "future_task" in tool_names:
        lines += [
            "",
            "## 定时任务（future_task）",
            "时间参数只接受两种写法，你要自己换算好再传：",
            "- 相对时间：+30m（30 分钟后）、+2h、+1d",
            "- 绝对时间：2026-08-02 08:00，或只给 08:00 表示最近的这个时刻",
            "用户说「明天早上八点」「半小时后」这类自然语言时，根据上面的当前时间算出对应写法。",
            "重复类型用 repeat 参数表达（once/hourly/daily/weekly），不要写进时间里。",
        ]

    # 对应 AstrBot _build_local_mode_prompt：告诉模型 shell 是 cmd 还是 POSIX，
    # 不然它在 Windows 上照样发 cat/ls/grep
    if os_name or shell_hint:
        lines += ["", "## 运行环境"]
        if os_name:
            lines.append(f"你运行在 {os_name} 上，可以执行 shell 命令和 Python 代码。")
        if shell_hint:
            lines.append(shell_hint)

    # 对应 AstrBot 的 workspace 约束（astr_main_agent.py:1631）
    if workspace:
        lines += [
            "",
            "## 工作目录",
            f"这次对话的工作目录是 `{workspace}`，它属于当前这个会话，别的群和别的人看不到。",
            "写文件直接用相对路径（比如 `report.md`）就会落在这里，不需要拼绝对路径。",
            "目录一开始可能不存在，你写第一个文件时会自动建好。",
            "这只是你的默认落脚点，不是唯一能去的地方：管理员额外授权过的目录同样可以读写，"
            "用户让你去处理别处的文件时，直接用那边的绝对路径。",
            "路径不被允许时工具会告诉你当前允许哪些目录，照它说的做，不要反复试同一个被拒的路径。",
        ]
        # 这段只在真有破坏性工具时才注入：普通用户拿不到文件写入和执行工具，
        # 给他们讲「改文件要确认」纯属浪费 token，还会让模型以为自己能改文件
        risky = [x for x in ("file_write", "file_edit", "execute_shell", "execute_python")
                 if x in tool_names]
        if risky:
            lines += [
                "",
                "## 改动已有文件前先确认",
                "你自己在工作目录里新建的文件，随便改。但下面这些情况要先问用户一句，"
                "得到明确答复再动手：",
                "- 覆盖或删除不是你这次创建的文件，尤其是用户自己的资料、代码、配置",
                "- 改动配置文件、脚本、程序源码——写坏了可能让别的东西跑不起来",
            ]
            if "execute_shell" in risky or "execute_python" in risky:
                lines.append("- 执行删除、移动、批量改名这类做完就撤不回来的命令")
            lines += [
                "确认时说清楚你要改哪个文件、改成什么样，用户明确同意了再执行。",
                "读取和查看不需要确认，直接看。",
            ]
        # 产出物必须有出口，否则模型会把文件路径念给用户，而用户根本拿不到
        deliver = []
        if "send_file" in tool_names:
            deliver.append("整理成的表格、报告、压缩包这类文件，用 send_file 发给用户下载")
        if "send_image" in tool_names:
            deliver.append("生成的图片（图表、渲染结果）用 send_image 发出去，用户能直接看到")
        if deliver:
            lines += [
                "",
                "## 把成果交给用户",
                "用户看不到这个目录，只念文件路径等于什么都没给他。做完之后：",
                *[f"- {x}" for x in deliver],
                "过程比较久时可以用 send_message 先汇报一句进展，别让用户干等。",
            ]

    # 刻意不再罗列工具名：每个工具的名字和用途已经在 tools 参数里给过一遍，
    # 提示词里再抄一份不会增加可用信息，反而容易让模型把「名字」当成能力凭空发挥
    # （比如看到 get_current_time 就去纠结时区、看到 read_bot_config 就想读配置）。
    return "\n".join(lines)
...
def _prune_overflow_dir(overflow_dir: str) -> None:
    ...
    try:
        ...
        for name in os.listdir(overflow_dir):
            ...
            try:
                ...
                continue
        cutoff = time.time() - OVERFLOW_KEEP_SECONDS
        entries.sort()
...
async def run_tool_loop(complete, messages: list[dict], ctx: AgentContext,
    ...
    """
    # 本次调用的采集容器，由内部实现就地填充
    collected: dict = {"calls": [], "llm_attempts": 0}
    try:
        return await _run_tool_loop_inner(complete, messages, ctx, settings, abort, collected)
    finally:
...
async def _run_tool_loop_inner(complete, messages: list[dict], ctx: AgentContext,
                               ...
                               collected: dict) -> tuple[str, list[dict], int]:
    ...
    # 把工具使用指南追加到系统提示词：模型需要知道当前时间、运行环境和格式约定，
    # 才能自己把「明天早上八点」换算成 future_task 认的写法、在 Windows 上发对命令。
    if messages and messages[0].get("role") == "system":
        names = [t["function"]["name"] for t in tools]
        # 只在真给了执行/文件类工具时才讲 shell 和工作目录，否则是无谓的 token
        ...
        guide = build_agent_guide(
            now_text=datetime.now().strftime("%Y-%m-%d %H:%M:%S %A"),
            is_group=ctx.is_group,
            user_level=ctx.user_level,
            tool_names=names,
            workspace=settings.workspace if needs_env else "",
            os_name=_OS_NAME if needs_env else "",
            shell_hint=SHELL_HINT if needs_env else "",
        )
        messages[0] = {
            "role": "system",
