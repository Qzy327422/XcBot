...
class TokenStats:
    ...
    def _prune_window_locked(self, force: bool = False) -> bool:
        ...
        注意：throttle 只跳过“扫描裁剪”，不能跳过“聚合一致性”。
        若调用方刚增量改了 total_*，而窗口外明细仍在，必须至少保证聚合可被后续 force 重建。
        """
        now = time.time()
        # 非强制时最多每 30 秒全量扫一次，避免每次 get_stats 都扫全表
        ...
        for sid in empty_sids:
            ...
            changed = True
        # 只要扫描过，就重建聚合，避免“明细已变/旧增量”和 total_* 不一致
    ...
    def add_usage(
        ...
        with self._lock:
            ...
            self._dirty = True
            # 以明细为唯一真相源重建，避免增量 + 节流 prune 造成 total 漂移
            self._rebuild_aggregates_locked()
