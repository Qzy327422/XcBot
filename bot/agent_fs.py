# -*- coding: utf-8 -*-
"""Agent 文件与执行工具 —— 搬自 AstrBot 的 file_read / write / edit / grep /
execute_shell / execute_python。

与 AstrBot 的关键差异：AstrBot 把 shell/python 放在 Docker 沙箱里跑，XcBot 没有
沙箱。这两个工具默认开启但只有管理用户能调用（项目只有普通用户/管理用户两档，
没有更高等级），风险在提示词和 WebUI 描述里都写明了。

文件工具全部是 admin 级，所以不再屏蔽含密钥的文件——管理员本来就能直接读写
整台机器，拦 config.json 只是徒增麻烦。但敏感文件判断本身要保留：
send_file / send_image 是 user 级，它们复用同一套路径解析（并额外限制在会话
工作区内），普通用户不能借它们把凭据文件发出去。
"""
...
# main.py 启动时注入，用于读 Agent.file_allowed_dirs / Agent.exec_timeout
...
def _is_sensitive_path(path) -> bool:
    ...
    for i, part in enumerate(parts):
        if part in _BLOCKED_DIRS:
            return True
        # _BLOCKED_PARTS 是 .git / __pycache__ 这类目录。_resolve_path 已经
        # 单独查过它，但 grep 的递归过滤只调 _is_sensitive_path，
        # 这里不认的话 .GIT/config 会被搜出来。
...
def _configured_roots() -> list[Path]:
    """管理员在 WebUI 配置的允许目录。留空表示没有额外授权。"""
    raw = _setting("Agent.file_allowed_dirs", []) or []
    roots = []
...
def _allowed_roots(ctx=None) -> list[Path]:
    """允许访问的根目录。

    当前会话的工作区永远在列（这是模型自己的地盘），管理员额外配置的目录追加在后。
    没有任何配置时就只有工作区，程序本体和 data 下的运行时数据都碰不到。
    ctx 为 None 时（WebUI 探测、旧调用点）退回到 shared 工作区。
    """
    roots = [session_workspace(ctx)]
    for extra in _configured_roots():
...
def primary_root(ctx=None) -> str:
    """文件工具的首选根目录，用于告知模型它的工作目录。

    必须和 _allowed_roots()[0] 保持一致：提示词里写项目根目录的话，
    模型会反复去试那些必然被拒的路径。
    """
    try:
        return str(_allowed_roots(ctx)[0])
    except Exception:
        return ""


def _resolve_path(raw: str, ctx=None, *, allow_sensitive: bool = False,
                  workspace_only: bool = False) -> tuple[Path | None, str]:
    """把模型给的路径解析成绝对路径，并校验它落在允许目录内。

    必须用 resolve() 之后再比较，否则 ../ 和符号链接都能穿越出去。
    相对路径基准是当前会话的工作区，不是程序目录——模型说「写 report.md」时
    应该落在自己的工作区里。

    allow_sensitive=True 跳过含密钥文件的屏蔽。只有 admin 级的文件工具能传这个：
    管理员本来就能读整台机器，拦 config.json 只是徒增麻烦。
    workspace_only=True 时把范围压到会话工作区，忽略管理员配置的额外目录。
    发文件/发图这类 user 级工具必须用它——否则普通用户能借工具把管理员
    授权目录里的东西发出来。
    """
    ...
    roots = [session_workspace(ctx)] if workspace_only else _allowed_roots(ctx)
    if not any(candidate == root or root in candidate.parents for root in roots):
        ...
        return None, (
            f"error: 路径 {candidate} 不在允许的目录内。当前允许：{allowed}（第一个是你本次会话的工作目录）。"
            "写文件时直接用相对路径就会落在工作目录里。"
            "如果确实需要访问别的目录，请如实告诉用户，让管理员在 WebUI 的 Agent 页面添加。"
        )

    if not allow_sensitive and _is_sensitive_path(candidate):
...
async def _tool_file_read(args: dict, ctx: AgentContext) -> str:
    path, err = _resolve_path(args.get("path"), ctx, allow_sensitive=True)
    if err:
        return err
    if not path.exists():
...
async def _tool_file_list(args: dict, ctx: AgentContext) -> str:
    ...
    try:
        for item in sorted(path.iterdir(), key=lambda p: (p.is_file(), p.name.lower())):
            if str(item.name).casefold() in _BLOCKED_PARTS:
                continue
            try:
                stat = item.stat()
    ...
    if path.is_file():
        ...
        for p in path.rglob(pattern_glob):
            try:
                if not p.is_file():
                    continue
                # rglob 的结果没经过 _resolve_path：允许目录里放一个指向外部
                # 文件的符号链接，grep 就能把外部内容捞回来，而 file_read
                # 读同一个目标会被拒。所以每个命中都要重新解析 + 校验根目录。
                real = p.resolve()
            except (OSError, RuntimeError):
                continue
            if not any(real == r or r in real.parents for r in roots):
                continue
            if _blocked_part_hit(real.parts):
                continue
            files.append(real)
