...
def check_webui_and_ui(js: str, css: str) -> None:
    ...
    # 按关键键存在校验，而不是固定条数，避免每次新增功能开关都要改断言
    feature_keys = {item["key"] for item in webui.FEATURE_META}
    ok(
        "webui feature meta",
        {"ai_chat", "plugins_external", "group_join_welcome", "http_send_api"} <= feature_keys,
    )
    ok(
        "webui feature defaults",
        webui.DEFAULT_FEATURE_SWITCHES.get("http_send_api") is True
        and webui.DEFAULT_FEATURE_SWITCHES.get("group_join_welcome") is True,
    )
    ok(
        "http send api grouped separately",
        any(x["key"] == "http_send_api" and x["group"] == "外部接口" for x in webui.FEATURE_META),
    )
