    ...
    def get_current(self, require_multimodal: bool = False) -> Optional[Tuple[str, str, str, bool]]:
        with self._lock:
            ...
            if default_idx is not None:
                ...
                # 修复 #4：require_multimodal=True 且默认 API 不支持多模态时，
                # 不要先切到默认再切走——会同时写两条切换日志，把 switch_logs 刷爆。
                # 这里直接跳过默认，进入下面的轮询逻辑。
                if not (require_multimodal and not bool(default_item.get("supports_multimodal", False))):
