...
# 与 config.default.json 的 config_version 保持同步；模板更高时以模板为准。
    ...
    for key, old_value in old_config.items():
        ...
        if key in result:
            ...
            if isinstance(old_value, dict) and isinstance(new_value, dict):
                ...
                # 保留用户列表（含空列表）；不要用 `if old_value` 把 [] 当成无值
    ...
    if isinstance(endpoints, list) and endpoints and _is_empty_provider_list(providers):
        ...
        for ep in endpoints:
            ...
            try:
                ...
                try:
                    timeout = int(float(others.get("api_request_timeout_seconds", 60) or 60))
                except (TypeError, ValueError):
                    timeout = 60
...
def write_json(path: Path, data: dict) -> None:
    """同目录临时文件 + os.replace，避免迁移中断留下半截 config。"""
...
def upgrade_group_join_welcome(config: dict, template: dict) -> bool:
    """只把未自定义的旧默认欢迎语升级为支持 {at} 的新版默认文案。"""
    others = config.get("Others") if isinstance(config, dict) else None
    template_others = template.get("Others") if isinstance(template, dict) else None
    if not isinstance(others, dict) or not isinstance(template_others, dict):
        return False
    current = others.get("group_join_welcome_text")
    new_default = template_others.get("group_join_welcome_text")
    if not isinstance(current, str) or not isinstance(new_default, str):
        return False
    if current.strip() != _OLD_GROUP_JOIN_WELCOME.strip() or current == new_default:
        return False
    others["group_join_welcome_text"] = new_default
    return True
...
def ensure_config_up_to_date(config_path: str = "config.json", template_path: str | None = None,
    ...
    welcome_migrated = upgrade_group_join_welcome(merged, template)
    merged[CONFIG_VERSION_KEY] = max(template_config_version, CURRENT_CONFIG_VERSION)
    # deep_merge 会保护 Others.version_name，升级时需显式同步到模板程序版本。
    program_version_changed = False
    if template_program_version:
        others = merged.get("Others")
        if not isinstance(others, dict):
            others = {}
            merged["Others"] = others
        if str(others.get("version_name", "") or "").strip() != template_program_version:
            others["version_name"] = template_program_version
            program_version_changed = True
    ...
    if (
        not version_upgraded
        and not missing_keys
        and not provider_migrated
        and not personality_migrated
        and not welcome_migrated
        and not legacy_migrated
        and not program_version_changed
    ):
        return False
...
def migrate(old_path: str, new_path: str, backup_dir: str, *, remove_old: bool = False):
    ...
    # 与 ensure_config_up_to_date 一致：程序版本号以新模板为准
    template_program_version = get_program_version(new_template)
    if template_program_version:
        others = merged.get("Others")
        if not isinstance(others, dict):
            others = {}
