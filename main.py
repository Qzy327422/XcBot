...
def normalize_llm_provider_rotation(cfg: dict) -> list:
    ...
    for raw_provider in providers:
        ...
        if isinstance(raw_models, list):
            for raw_model in raw_models:
                ...
                try:
                    ...
                    try:
                        timeout_seconds = int(float(cfg.get("api_request_timeout_seconds", 60) or 60))
                    except (TypeError, ValueError):
                        timeout_seconds = 60
                if name in {m.get("name") for m in models}:
                    print(f"[模型配置] 已忽略渠道 {provider_id} 的重复模型：{name}")
                    continue
                models.append({
                    "name": name,
        ...
        if provider_id in provider_map:
            print(f"[模型配置] 已忽略重复渠道 ID：{provider_id}，请在 WebUI 中改成唯一名称")
            continue
...
class ChatMemoryManager:
    ...
    def __init__(self):
        ...
        self.memory_path = os.path.join(str(BASE_DIR), "data", 'ai_memory')
