...
try:
    # 让自动更新走重启路径时也能先释放进程锁，避免新进程被锁挡在外。
...
def normalize_llm_provider_rotation(cfg: dict) -> list:
    """按提供商/模型轮换配置生成 key_manager 使用的模型槽。"""
    cfg = cfg or {}
    providers = cfg.get("llm_providers", [])
    rotation = cfg.get("llm_rotation", [])
    legacy_endpoints = cfg.get("llm_endpoints", [])
    ...
    if not provider_has_model and isinstance(legacy_endpoints, list):
        converted = []
        for index, ep in enumerate(legacy_endpoints, start=1):
            ...
            converted.append({
                ...
                "models": [{
                    ...
                    "enabled": True,
                    "supports_multimodal": normalize_bool_config(ep.get("supports_multimodal", False), False),
                    "timeout_seconds": cfg.get("api_request_timeout_seconds", 60),
                }],
                "detected_models": [],
            })
        providers = converted
    ...
    for raw_provider in providers:
        ...
        if not provider_id or not base_url:
            continue
        keys = raw_provider.get("keys", [])
        if isinstance(keys, str):
            keys = [x.strip() for x in keys.splitlines() if x.strip()]
        ...
        for key in keys:
            lowered = key.lower()
            if any(token in lowered for token in ["your_api_key", "api_key", "sk-xxxx", "示例", "占位"]):
                continue
            try:
                key.encode("ascii")
            except UnicodeEncodeError:
        ...
        if isinstance(raw_models, list):
            for raw_model in raw_models:
                if isinstance(raw_model, str):
                    raw_model = {"name": raw_model, "enabled": True}
                ...
                if not name:
                    continue
                try:
                    timeout_seconds = int(float(raw_model.get("timeout_seconds", cfg.get("api_request_timeout_seconds", 60)) or 60))
                except (TypeError, ValueError):
                    timeout_seconds = int(cfg.get("api_request_timeout_seconds", 60) or 60)
                models.append({
                    "name": name,
                    "enabled": normalize_bool_config(raw_model.get("enabled", True), True),
                    "supports_multimodal": normalize_bool_config(raw_model.get("supports_multimodal", False), False),
    ...
    for item in rotation:
        ...
        ref = (provider_id, model)
        if provider_id and model and ref not in seen_refs:
            seen_refs.add(ref)
        ...
        for provider_id, provider in provider_map.items():
            for model in provider.get("models", []):
                if model.get("enabled"):
                    ref = (provider_id, model["name"])
                    if ref not in seen_refs:
                        seen_refs.add(ref)
                        ordered_refs.append(ref)

    normalized = []
    for provider_id, model_name in ordered_refs:
        ...
        for model in provider.get("models", []):
            if model.get("name") == model_name:
                model_cfg = model
                break
        if not model_cfg or not model_cfg.get("enabled"):
            continue
        normalized.append({
            "provider_id": provider_id,
            "base_url": provider["base_url"],
            "model": model_name,
            "display_model": f"{provider_id}/{model_name}",
            "keys": provider["keys"],
            "supports_multimodal": normalize_bool_config(model_cfg.get("supports_multimodal", False), False),
            "timeout_seconds": int(model_cfg.get("timeout_seconds", cfg.get("api_request_timeout_seconds", 60)) or 60),
        })
...
def cleanup_old_quote_temps(max_age_seconds: int = 300) -> None:
    """修复 #3：Quote 生成 temps/quote_<uuid>.png，但原有清理只删硬编码的 quote.png。
    这里按 mtime 清理 5 分钟以上的旧 quote 图，避免 temps/ 目录无限膨胀。"""
    try:
        now = time.time()
        for name in os.listdir("./temps"):
            if not name.startswith("quote_") or not name.endswith(".png"):
                continue
            full = os.path.join("./temps", name)
            try:
...
def load_chat_db():
    ...
    json_path = os.path.join("data", 'sum_up', 'chat_db.json')
...
失败: 未能获取到 {uid} 的信息，可能不是一个有效的用户。'''
    ...
    if "NapCat" in framework.get("app_name"):
        avatar, r = parser_user_info_napcat(user_info, ROOT_User)
    else:
        avatar, r = parse_user_info(user_info, ROOT_User)
...
def parser_user_info_napcat(user_dict, admin_users):
    try:
        ...
        status_msg = "(框架不支持)"
        if str(user_dict.get('user_id', '未知')) in admin_users:
            status_user = "管理员"
        else:
            status_user = "普通用户"
...
def parse_user_info(user_dict, admin_users):
    try:
        avatar = user_dict.get('avatar', '')
...
class TokenStats:
    ...
    def __init__(self):
        ...
        self._reset_date = date.today().isoformat()
...
async def handle_token_command(event, actions, is_group=True, order=""):
    ...
    elif "重置token统计" in order and is_admin_user(user_id):
        token_stats.reset()
        msg = "✅ 全局Token统计已重置"
        if is_group:
...
async def handle_compression_commands(event, actions, is_group=True, order=""):
    ...
    elif "全部压缩状态" in order and is_admin_user(user_id):
...
{reminder}token统计 —— 查看 Token 统计
{reminder}重置token统计 —— 重置 Token 统计（仅管理员）
{reminder}感知 —— 查看运行状态（仅管理员）
{reminder}开 [QQ号] —— 查询 QQ 资料
{reminder}重载插件 / 禁用插件 / 启用插件 / 插件视角 —— 插件管理（仅管理员）
{reminder}model / modeldefault / modellog —— 模型管理（仅管理员）
...
你拥有管理{bot_name}的权限。
    ...
    8. {reminder}token统计 —— 查看 Token 消耗统计
    9. {reminder}重置token统计 —— 重置全局 Token 统计
    ...
    16. {reminder}modeldefault —— 查看当前默认模型
    17. {reminder}modeldefault <编号|模型名> —— 设置默认模型
    18. {reminder}modeldefault clear —— 清除默认模型设置
    19. {reminder}modellog —— 查看最近 API 切换日志
    20. {reminder}启用model <编号> —— 手动恢复被禁用的 API
    21. {reminder}重置model冷却 <编号> —— 清除某个 API 的冷却状态
你的每一步操作，与用户息息相关。'''
            else:
                content = "仅管理员可操作"
...
def restart_current_process(reason: str = "配置变更"):
    ...
    # 修复 #2：os.execv 不会触发 atexit，必须手动释放进程锁，
    # 否则新进程启动时 msvcrt.locking / fcntl.flock 会抢不到锁直接 sys.exit(1)。
    try:
        release_lock()
...
if __name__ == "__main__":
    try:
        cleanup_legacy_config_files()
        Read_Settings()
        # 自动更新重启前释放进程锁，否则新进程会被 my_bot.lock 挡掉。
