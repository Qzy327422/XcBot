...
class LimitedDeepSeekContext:
    ...
    async def agen_content(self, message) -> tuple[str, int, int, int]:
        ...
        for attempt in range(max_retries):
            use_multimodal_directly = isinstance(message, dict) and bool(message.get("image_urls")) and bool(key_manager.is_default_multimodal())
            # 默认/主模型支持多模态时直接看图；否则仍优先走主模型，先用配置的多模态模型转述图片。
        ...
        try:
            ...
            if running:
                ...
                if running and retry_count < max_retries:
                    wait_time = retry_delay * max(retry_count, 1)
                    print(f"连接断开，等待 {wait_time} 秒后重试...")
                    print("-" * 30)
