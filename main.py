...
系统提示词: ✅ 独立存储'''
        ...
        try:
            ...
            if _has_active_session(_p_session_id):
                _follow_up_session(_p_session_id, final_message)
                deepseek_context.history.append({
                    "role": "user",
                    "content": filter_sensitive_content(final_message)
                })
        ...
        try:
            ...
            if _has_active_session(_p2_session_id):
                _follow_up_session(_p2_session_id, final_message)
                deepseek_context.history.append({
                    "role": "user",
                    "content": filter_sensitive_content(final_message)
                })
        ...
        if should_trigger and order:
            ...
            try:
                ...
                if _has_active_session(_g_session_id):
                    _follow_up_session(_g_session_id, final_message)
                    deepseek_context.history.append({
                        "role": "user",
                        "content": filter_sensitive_content(final_message)
                    })
