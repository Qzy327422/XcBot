...
def _agent_local_media_path(raw: str, ctx):
    """把模型给的本地路径解析成会话工作区内的真实文件。

    返回 (Path, "")；不是本地路径时返回 (None, "")，出错时返回 (None, error 文本)。
    复用文件工具的 _resolve_path，但强制 workspace_only —— send_file / send_image
    都是 user 级，如果跟着 file_allowed_dirs 走，普通用户就能让 AI 把管理员
    授权目录里的任意文件发到群里，绕过 file_read 的 admin 限制。
    敏感文件屏蔽同样保留：这里不是管理员通道。
    """
    ...
    path, err = _agent_fs_tools._resolve_path(text, ctx, workspace_only=True)
    if err:
        return None, err
    if not path.exists():
        return None, (
            f"error: 文件不存在：{path}。"
            "只能发送你自己在工作目录里创建出来的文件，请先确认它已经写成功。"
        )
    if not path.is_file():
...
@agent_tool(
    ...
    description=(
        ...
        "② 你自己在工作目录里生成的本地图片，直接填文件名或相对路径即可"
        "（比如用 execute_python 画出的图表）。"
    ),
    parameters={
        "type": "object",
        "properties": {
            "image_url": {"type": "string", "description": "图片直链（http/https），或工作目录里的图片文件路径"},
        },
        "required": ["image_url"],
    },
    side_effect=True,
...
@agent_tool(
    name="send_file",
    description=(
        "把你在工作目录里生成的文件发送给用户，比如整理好的表格、报告、导出的数据、"
        "打包好的压缩包。填文件名或相对路径即可。\n"
        "群聊里会上传到群文件，私聊里会作为文件发给对方。\n"
        "只能发工作目录里的文件——你自己写出来的东西。想发图片让用户直接看到，"
        "用 send_image 更合适；这个工具是发可下载的文件。"
    ),
    parameters={
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "工作目录里的文件路径，可以只写文件名"},
...
async def _agent_send_file(args: dict, ctx) -> str:
    ...
    if path is None:
        return (
            "error: send_file 只能发送工作目录里的本地文件，不能发网络链接。"
...
class EnhancedLimitedDeepSeekContext(LimitedDeepSeekContext):
    ...
    async def agen_content(self, message, agent_meta: dict | None = None) -> tuple[str, int, int, int]:
